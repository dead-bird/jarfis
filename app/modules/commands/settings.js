...
let self = module.exports = {
  ...
  list(client, msg) {
    ...
    let embed = new Discord.RichEmbed()
      .setColor(3447003)
      .setDescription(`:joystick:  Run \`${options.prefix}settings change [setting name] [value]\` to edit`);
  
    for (let setting in self.config) {
      embed.addField('\u200B', `**${self.config[setting].name}** - ${self.config[setting].list(options[setting])}`);
    }
  ...
  change(client, msg, setting, value) {
    ...
    core.server.get(client, msg.guild, options => {
      if (!(setting in options)) return core.err.reply(msg, `hmm I don't think I have a setting called \`${setting}\`, maybe get good?`);
      ...
      self.config[setting].change(value, option => {
        ...
        msg.reply(`I've set \`${setting}\` to ${value}`).catch(err => core.err.dead(msg, err));
      });
    });
  },
